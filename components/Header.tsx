...
const Header = () => {
  ...
  const handleNavClick = (e) => {
    ...
    // Un pequeño retardo para asegurar que el menú se cierre antes del desplazamiento.
    setTimeout(() => {
      ...
      if (targetElement) {
        ...
        let targetPosition;
        if (targetId === '#home') {
          // Caso especial para el enlace "Home", desplaza al inicio de la página.
          targetPosition = 0;
        } else {
          // Calcula la posición del elemento de destino, restando la altura de la cabecera.
          const elementPosition = targetElement.getBoundingClientRect().top;
          targetPosition = elementPosition + window.pageYOffset - headerHeight;
        }
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 shadow-md backdrop-blur-sm' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          <a href="#home" onClick={handleNavClick} className={`text-2xl font-bold transition-colors cursor-pointer ${isScrolled ? 'text-cyan-800' : 'text-white'}`}>
            {t('header_title')}
          </a>
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {navLinks.map((link) => (
              <a key={link.href} href={link.href} onClick={handleNavClick} className={`text-lg font-medium transition-colors hover:text-cyan-600 cursor-pointer ${isScrolled ? 'text-gray-700' : 'text-white'}`}>
                {link.label}
              </a>
            ))}
          </nav>
        ...
        {isMenuOpen && (
          <div className="md:hidden mt-4 bg-white rounded-lg shadow-xl">
            <nav className="flex flex-col items-center space-y-4 p-4">
              {navLinks.map((link) => (
                <a key={link.href} href={link.href} onClick={handleNavClick} className="text-lg font-medium text-gray-700 transition-colors hover:text-cyan-600 cursor-pointer">
                  {link.label}
                </a>
              ))}
            </nav>
